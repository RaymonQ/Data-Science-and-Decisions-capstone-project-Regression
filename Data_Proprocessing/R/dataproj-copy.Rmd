...
library("plyr")
temp.data <- data.frame(gliders_valid$TEMP)
head(temp.data)
nrow(temp.data)
nrow(na.omit(temp.data))/nrow(temp.data)
ran <- sample(1:nrow(temp.data), 0.9 * nrow(temp.data))
nor <-function(x) { (x -min(x))/(max(x)-min(x))   }
temp_norm <- as.data.frame(lapply(temp.data[,c(1)], nor))
summary(temp_norm)
...
method 2
```{r}
tem <- data.frame(scale(gliders_valid$TEMP))
tem <- cbind(tem, tem[, 1])
names(tem)[1] = "Species"
sample.list <- sample(1:150, size = 100)
tem.known <- tem[sample.list, ]
tem.unknown <- tem[-sample.list, ]
length.known <- nrow(tem.known)
length.unknown <- nrow(tem.unknown)
for (i in 1:length.unknown) { 
    dis_to_known <- data.frame(dis = rep(0, length.known)) 
    for (j in 1:length.known) { 
        dis_to_known[j, 1] <- dist(rbind(tem.unknown[i, 1], tem.known[j,1]), method = "euclidean") 
        dis_to_known[j, 2] <- tem.known[j, 1]
    ...
    tem.unknown[i, 1] <- type_freq[1, 1]
}

names(tem.unknown)[1] = "Species.pre"
...
method 3:
```{r}
tem <- data.frame(gliders_valid$TEMP)
data(tem)
m <- dim(tem)[1]
